...
// RETURNIN clause to read them into the User struct after the insert, in the same way
...
// Retrieve the User details from the database based of the user's email address.
// Because we have a UNIQUE constraint on the email column, this SQL query will only
// return one record (or non at all, in which case we return a ErrRecordNotFound error).
